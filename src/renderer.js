...
async function startRecording() {
  ...
  setStatus('錄影中: 可在原始畫面畫筆標註（Shift 單擊切換繪製，滾輪會短暫暫停畫筆） | 畫質: ' + qualityPreset.label + ' (' + audioMode + ')');
}
...
async function setPenMode(enabled) {
  ...
  try {
    ...
    if (enabled && mode && mode.toggleMode) {
      ...
      penToggleBtn.textContent = '畫筆模式: 開（Shift 單擊切換；滾輪暫停 ' + pauseMs + 'ms）';
      return;
    }
  } catch (_error) {
  }

  penToggleBtn.textContent = enabled ? '畫筆模式: 開（Shift 單擊切換）' : '畫筆模式: 關';
}
